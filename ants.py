...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place_location = 0
        while ent != beehive: 
            if ent.bees != [] and (self.min_range <= place_location <= self.max_range):
                return rANTdom_else_none(ent.bees)
            else:
                ent = ent.entrance
                place_location += 1
        return None      
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > 0:
            for bee in self.place.bees[:]:
                super().reduce_armor(bee, amount)
        else:
            for bee in self.place.bees[:]:
                super().reduce_armor(bee, amount+self.damage)
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 5
